(function() {
  ...
  function reloadReport() {
    ...
    ajaxGet("/api/1.0/website?m=" + currentManufacturer).done(function(data) {
      websites = data;
      renderTable()
  ...
  function loadPartsPerCostPriceRange() {
    ajaxGet("/api/1.0/parts/per_cost_price_range?m=" + currentManufacturer + "&t=" + currentPartType).done(function(data) {
      ranges = data;
      renderTable();
      loadAllPartPrices();
    })
  ...
  function loadPartPrice(partNumber, range) {
    ajaxGet("/api/1.0/part/" + partNumber + "/pricing_on_date?d=" + currentDate).done(function(data) {
  ...
  function handleExcludeWebsite() {
    ...
    ajaxPut("/api/1.0/website_exclusions", { excluded_website_ids: excludedIds }).done(function() {
      onFilterChange();
    })
  }
